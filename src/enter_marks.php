...
if ($year && $term) {
    ...
    if ($batch_id) {
        ...
        $batch = $batchModel->findOrCreate($year, $term, $teacher['stream_id']);
    }
    if ($batch) {
        $batch_id = $batch['id'];
    } else {
...
<div class="card mb-4">
    <div class="card-body">
        <form action="?page=enter_marks" method="get" class="row g-3 align-items-end">
            ...
            <div class="col-md-4">
                <button type="submit" class="btn btn-primary w-100">Load Marks Sheet</button>
            </div>
        </form>
    </div>
</div>
...
<?php if ($batch):
    ...
    $students = $studentModel->getAllByStream($teacher['stream_id']); // Note: Need to implement getAllByStream
